...
class BTreeFile: public IndexFile
{
  public:
    ...
    Status splitIndexPage(PageId splitpageid, BTIndexPage* splitpage, void* &splitkey, PageId& newpageid,
    		BTIndexPage*& newpage, void*& newkey, const void* insertkey,int newdatapageno,AttrType keytype);

    Status splitLeafPage(PageId splitpageid, BTLeafPage* splitpage, void* &splitkey, PageId& newpageid,
        		BTLeafPage*& newpage, void*& newkey, const void* insertkey, RID data,AttrType keytype);

    Status findLeaf(const void* key,PageId& leafid, vector<PageId>& indexlist, BTLeafPage*& leafpage);

    void content(PageId testpageno);
