...
// cout<<"Scan constructor works"<<endl;
  status = OK;
}
...
Status Scan::getNext(RID& rid, char *recPtr, int& recLen)
{
  ...
  if (userRid.slotNo==-1)//first time run this func
  {
  ...
  else
  {
    ...
    if (nxtUserStatus==0){//next record doesn't exist
      status=nextDataPage();
      if (status==DONE)
      {
  ...
  //determine whether the next record exists
  status=dataPage->nextRecord(userRid,nextRid);
  if (status!=OK)
  {
    nxtUserStatus=0;
...
{
  //position the scan cursor to the record
  userRid = rid;
  // it can't be non-OK
  return OK;
}
...
Status Scan::mvNext(RID& rid)
{
  ...
  if (userRid.slotNo==-1)//first time run this function
  {
    status=dataPage->firstRecord(userRid);
  }
  else
  ...
  //return the RID of the (new) current record
  rid=userRid;
